                        ...
                        <div class="col-md-6 mar-top-30">
                            ...
                            <div class="cart-subtotal">
                                ...
                                <div class="shipping">
                                    ...
                                    @if ($setting_free_shipping->shipping_status==1)
                                        <div class="custom-control custom-radio mt-3">
                                            <input type="radio" @if($setting_free_shipping->minimum_amount == $shipping_charge) checked @endif data-shipping_type='free' name="shipping_cost" class="custom-control-input shippingCharge" value="{{$setting_free_shipping->minimum_amount ?? 0}}">
                                    ...
                                    @if ($setting_local_pickup->pickup_status==1)
                                        <div class="custom-control custom-radio mt-3">
                                            <input type="radio" @if($setting_local_pickup->cost == $shipping_charge) checked @endif data-shipping_type='local_pickup' name="shipping_cost" class="custom-control-input shippingCharge" value="{{$setting_local_pickup->cost ?? null}}">
                                    ...
                                    @if ($setting_flat_rate->flat_status==1)
                                        <div class="custom-control custom-radio mt-3">
                                            <input type="radio" @if($setting_flat_rate->cost == $shipping_charge) checked @endif data-shipping_type='flat_rate' name="shipping_cost" class="custom-control-input shippingCharge" value="{{$setting_flat_rate->cost ?? null}}">
                                ...
                                <div class="total">
                                    ...
                                    <div class="price">
                                        @if(env('CURRENCY_FORMAT')=='suffix')
                                            <span class="cart_total total_amount">{{$cart_total}}</span> @include('frontend.includes.SHOW_CURRENCY_SYMBOL')
                                        @else
                                            @include('frontend.includes.SHOW_CURRENCY_SYMBOL') <span class="cart_total total_amount">{{$cart_total}}</span>
                                        @endif
                                    </div>
                                </div>
                                <hr>
    ...
    $('#billing_country').change(function() {
        ...
        $.ajax({
            ...
            success: function (data) {
                var total = data.total_amount* {{$CHANGE_CURRENCY_RATE}};
    ...
    $('.shippingCharge').on("click",function(e){
        ...
        $.ajax({
            ...
            success: function (data) {
                console.log(data)
                if (data.type=='success') {
                    var total = data.total_with_shipping * {{$CHANGE_CURRENCY_RATE}}
                    $('.total_amount').text(total.toFixed(2));
                    $('#totalAmount').val(data.total_with_shipping); //for from
                }
            }
        })
    });
