@extends('admin.main')
@section('title',' Report | Search Keyword')
@section('admin_content')


<section>
    <h4 class="mb-3 ml-4 text-bold">@lang('file.Search Report')</h4>
        ...
        <div class="row">
            <div class="col-md-8">
                <table id="datatable1" class="table">
                    <thead>
                        <tr>
                            <th class="wd-15p">@lang('file.Keyword')</th>
                            <th class="wd-15p">@lang('file.Hits')</th>
                        </tr>
                    </thead>
                    <tbody>
                        @forelse ($keyword_hits as $item)
                            <tr>
                                <td>{{$item->keyword}}</td>
                                <td>{{$item->hit}}</td>
                            </tr>
                        ...
                        <form action="{{route('admin.reports.search_report')}}" method="get">
                            ...
                            <div class="form-group mt-4">
                                ...
                                <select name="report_type" class="form-control selectpicker" data-live-search="true" data-live-search-style="begins" onchange="location = this.value;">
                                    ...
                                    <option value="{{route('admin.reports.tax_report')}}">@lang('file.Product Purchase Report')</option>
                                </select>
                            </div>
                        </form>
                    </div>
